...
| `plugin_recovered` / `caption_recovered` | The second and third Facebook post surfaces. `plugin_recovered` is Meta's embed fragment, which measured 33 of 35 sampled post urls on 2026-08-12 and carries every `/photo/?fbid=` url the page surface answers with a login wall. `caption_recovered` is the narrow last resort — a page with a byline and a caption and no `og:image` — and measured 2 of those 35, so it should stay small. | each other, and `fb`/`ok` |
