...
below, and nobody has run it that way. Implementation is parked with no timeline.

The audit reads the code and finds no *known* blocker in it, a claim about the code and not a
demonstration of it, and every claim below cites its file. Whether a self-hosted instance draws the
same cards is unmeasured; the egress table measures ten surfaces for whether the answer changes with
the IP the fetch leaves from, and three of the ten answer the same either way.

An earlier revision of this project's notes said self-hosting was not achievable, and the README
comparison table said "Workers only". `README.md:101` now carries the corrected row and links here.

---

## The container, on its own

`container/` is self-hostable today with no changes, and nobody has run it that way. From the
repository root, with a Docker-compatible CLI, no Cloudflare account and no `wrangler` login:
...
`/health` answers `200 ok` once the image is up.
...
bundler, no `wrangler`, no network. Node strips the TypeScript types itself, so `src/worker.ts` is
imported as written.

Re-measured on this tree 2026-08-05: 1207 tests, 0 failures, 19.3-20.1 s across runs, on Node
v26.5.0. That replaces 1185 tests in 19.9 s, measured 2026-08-04. CI pins Node 22.18.0
(`.node-version`) and runs the same command. Its comment reads "No network, no container, no R2 —
...
They drive the media/mux path, the translation deadline, the YouTube date path, the media proxy and
`/_prep`, with object literals standing in for the bindings. That proves the module graph loads and
executes under Node, and nothing more.

### The adapter entry point

`env`, `ctx` and `Deps` are ordinary parameters of `handle()` at `src/worker.ts:3358`:
...
Both are used for their shape alone: the suite fakes them with object literals and runs the entire
`/_media/` mux path against them under Node:
...
`Deps` (`src/worker.ts`), and the runtime cache never appears inside `handle`. `ExecutionContext` is
...
a static Deno binary, `EXPOSE 8080`, `CMD ["python", "server.py"]`, and no part of that needs
porting. The image has not been re-exercised outside Workers, and `container/README.md` documents
only the `wrangler deploy` path.

#### The `/resolve` contract

From the module docstring in `container/server.py` and the `do_POST` handler. **`POST /resolve`**,
JSON body, three shapes:
...
Any shape also accepts an optional `"cookies"`, the contents of a Netscape `cookies.txt`. The jar is
...
**Environment** (`container/server.py:46-53`, and `PORT` read once at `:429`): `PORT` (8080),
`RESOLVER_SECRET` (unset = no auth), `MAX_SECONDS` (1500), `MAX_BYTES` (393216000, a 375 MB output
ceiling), `PROC_TIMEOUT` (120 s per subprocess). `server.py` is authoritative on these numbers
whenever `container/README.md` disagrees, and the README has carried a stale pair before. `MAX_SECONDS` and `MAX_BYTES` were raised together on
2026-08-03 (`container/server.py:48`) and have to move together. The mux is `-c copy`, so output size
is the source bitrate times the duration.
...
Platforms answer a datacenter IP differently from a residential one, in both directions, and the
address the fetches leave from decides what a self-hosted instance can draw. Each row cites its
source and the date it was measured.
...
| Twitter guest endpoints | Byte-identical from residential and Cloudflare Workers egress in the 2026-07-19 recon; the activation and `TweetResultByRestId` were both measured working from Workers egress. | `src/platforms/twitter/fetch.ts`, `:129`, `:147` |
...
and would probably avoid the ~81 KB Instagram shell. Threads video bytes may be fetchable directly,
which mbedfx does not attempt. Reddit's anonymous `.json` would most likely stay blocked, that block
looking as much like bot detection as datacenter detection, and nothing measures it either way.
Google's limit is on burst rate and not on address, so a move buys nothing there. An instance with a
cold cache bursts harder than the public one.

A VPS or cloud host sits in the same category as the public instance, possibly worse. A smaller
provider's ranges are often blocked more aggressively than Cloudflare's, and none of the ten rows
above transfer to a different datacenter operator. Every one was taken from a residential laptop or
from Cloudflare, none from AWS, Hetzner, a home server or anywhere else. Whether a self-hosted
instance renders the same cards is unknown until somebody takes that measurement.

`translate_fallback`, `copyright_gql` / `copyright_recovered` / `copyright_remux` and
`fullpage_recovered` exist to make an egress-wide block visible. In each case the cards keep
...
|---|---|---|---|---|
| `ASSETS` | `src/analytics.ts:91` | one call: `env.ASSETS.fetch(req)` (`src/worker.ts:3373`) | any static file server over `public/`, wrapped to take a `Request` and return a `Response` | Required, about six lines of Node. Only the landing page and the converter page touch it, and both throw without it. |
...
| `MEDIA_RESOLVER` (Durable Object) | `src/analytics.ts:95` | `resolver.getByName(name).fetch(url, init)` and nothing else | an object whose `getByName(name)` returns `{ fetch }` pointed at the container. The name is a pooling slot (`resolver-{generation}-{0..N}`), so it can map to one container or several | Optional. Without it or without `MEDIA_CACHE`, `withResolver` strips every `remux` video and those platforms render a cover still, the same as before playback existed (`src/worker.ts`). |
...
self-hosted box it is a self-request loop and an internal-service reach. The test is
`host === h || host.endsWith('.' + h)`, so one apex entry per zone covers every subdomain.
`fetchableInstance` also takes an optional `self` origin, and the Worker threads none in
(`src/worker.ts`). Doing so would make the guard depend on a request field every test stub would then
have to supply.

Whatever domain you serve from has to be in that list. The adapter phase should make it
configurable.
...
`container/server.py` already resolves each host and refuses private, loopback, link-local, reserved
and multicast addresses. The Worker half needs the same check, which nobody has written; Cloudflare
has never needed it.
...
No code required. The three commands above stand the remux and resolve service up on any Docker
host, with `RESOLVER_SECRET` set.
...
- a `ctx` whose `waitUntil` tracks the promise and keeps the process alive until it settles. Make it
  a no-op and videos never appear;
