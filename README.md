...
[CONTRIBUTING.md](.github/CONTRIBUTING.md) has the rest of the layout, the commands, the test count and why `npm run deploy` refuses on purpose. [docs/SELF-HOSTING.md](docs/SELF-HOSTING.md) walks `handle()` at `src/worker.ts:4366`, the eight Cloudflare surfaces behind it and `container/server.py`.
